...
CREATE TABLE IF NOT EXISTS message ( sender_id VARCHAR(10), channel_id VARCHAR(5), message_id INTEGER PRIMARY KEY AUTOINCREMENT, timestamp DATETIME NOT NULL, content TEXT NOT NULL, message_type VARCHAR(1), FOREIGN KEY (sender_id) REFERENCES user(user_id) ON UPDATE CASCADE ON DELETE CASCADE, FOREIGN KEY (channel_id) REFERENCES channel(channel_id) ON UPDATE CASCADE ON DELETE CASCADE);
